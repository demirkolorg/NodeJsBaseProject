...
router.post('/update', async (req, res, next) => {
    ...
    try {
        ...
        if (body.permissions && Array.isArray(body.permissions) && body.permissions.length > 0) {
            ...
            let newPermissions = body.permissions.filter(p => !permissions.map(s => s.permission).includes(p))

            if (removePermissions.length > 0) {
                await RolePrivileges.deleteMany({ _id: { $in: removePermissions.map(I => I._id) } })
            }
