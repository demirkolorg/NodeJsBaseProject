...
router.post('/add', async (req, res, next) => {
  ...
  try {
    ...
    res.json(Response.successResponse(createdUser,
...
router.post('/update', async (req, res, next) => {
  ...
  try {
    ...
    if (Array.isArray(body.roles) && body.roles.length > 0) {
      ...
      let removedRoles = userRoles.filter(r => !body.roles.includes(r.role_id))
      let newRoles = body.roles.filter(p => !userRoles.map(r => r.role_id).includes(p))

      if (removedRoles.length > 0) {
        await UserRoles.deleteMany({ _id: { $in: removedRoles.map(I => I._id) } })
      }
...
router.post('/delete', async (req, res, next) => {
  ...
  try {
    ...
    res.json(Response.successResponse(removedUser,
